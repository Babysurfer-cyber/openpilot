...
class CarrotServ:
  ...
  def update_navi(self, remote_ip, sm, pm, vturn_speed, coords, distances, route_speed, gps_service):
    ...
    if my_driving_mode == 5:
      ...
      if auto_is_cam:
        ...
        if self.prev_speed_limit <= 0:
          current_limit = 0
        elif auto_raw_limit > 0 and auto_raw_limit < self.prev_speed_limit:
    ...
    # =========================================================
    if my_driving_mode == 5:
      # 오토모드: 모델 커브 속도에 여유를 주고, 내비게이션(vturn) 조건 삭제
      model_turn_speed = max(sm['modelV2'].meta.modelTurnSpeed, self.autoCurveSpeedLowerLimit) * 1.05
      if model_turn_speed < 200:
        speed_n_sources.append((model_turn_speed, "MODEL"))
    else:
      # 기존 모드: 오리지널 로직 유지
      model_turn_speed = max(sm['modelV2'].meta.modelTurnSpeed, self.autoCurveSpeedLowerLimit) * 1.1
      if model_turn_speed < 200 and abs(vturn_speed) < 60:
        speed_n_sources.append((model_turn_speed, "MODEL"))
