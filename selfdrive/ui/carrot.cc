...
public:
    void draw(const UIState* s, int x, int y) {
        ...
        // radarState가 추가로 필요하므로 alive 체크에 추가합니다.
        ...
        // --- 사각지대 위험 여부 판단 로직 (BlindSpotDrawer와 동일한 기준 적용) ---
        ...
        // 내 속도의 3배 거리 이내에 옆차선 차량이 다가오거나 사각지대 센서가 켜졌을 때 위험(unsafe)으로 간주
        bool left_unsafe = left_blindspot || (lead_left.getStatus() && lead_left.getDRel() < car_state.getVEgo() * 3.0);
...
public:
    ...
    void drawHud(UIState* s) {
        ...
        if (show_device_state) {
            ...
            // 레이아웃 위치 및 간격 설정 (전체적으로 오밀조밀하게 줄임)
            dx = s->fb_w - 100;       
            dy = 280;                // 전체 세트 시작 위치 살짝 올림
            int label_y_gap = 50;    // 항목 이름(DISK)과 숫자 사이의 상하 간격 (기존 55 -> 50)
            int step_y = 115;        // 각 항목 그룹 사이의 간격 (기존 130 -> 115)
            ...
            // 5. TIRE (타이어 공기압)
            // 🚨 타이어는 값이 2줄이므로, 다른 항목들과 똑같은 시각적 여백을 갖도록 30픽셀(한 줄 높이)을 더 내려줍니다.
            dy += step_y + 30; 
            ui_draw_text(s, dx, dy - 80, "TIRE", 40, COLOR_WHITE, BOLD, 0.0f, 0.0f);
            ...
            // 요청하신 대로 / 기호 양옆으로 공백(스페이스바)을 하나씩 더 추가했습니다!
            sprintf(line1, "%s  /  %s", fl_str, fr_str); 
            sprintf(line2, "%s  /  %s", rl_str, rr_str); 

            // 폰트 크기 40으로 적용하고 위아래 2줄 그리기
            ui_draw_text(s, dx, dy - 40, line1, 40, COLOR_WHITE, BOLD, 0.0f, 0.0f);
            ui_draw_text(s, dx, dy, line2, 40, COLOR_WHITE, BOLD, 0.0f, 0.0f);
        }
