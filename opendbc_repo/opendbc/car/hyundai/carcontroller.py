  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.flags & HyundaiFlags.CANFD:
      ...
      if self.frame % 5 == 0 and (not hda2 or hda2_long or camera_scc):
        can_sends.extend(hyundaicanfd.create_lfahda_cluster(self.packer, CS, self.CAN, CC.longActive, CC.latActive, CC.hudControl))
        if not camera_scc:
      ...
      if self.CP.openpilotLongitudinalControl:
        ...
        if True: #not camera_scc:
          can_sends.extend(hyundaicanfd.create_ccnc_messages(self.CP, self.packer, self.CAN, self.frame, CC, CS, hud_control, apply_angle, left_lane_warning, right_lane_warning, self.enable_corner_radar, stopping, self.canfd_debug, self.sm))
          if hda2:
