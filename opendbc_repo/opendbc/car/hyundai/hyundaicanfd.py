  ...
  if CP.flags & HyundaiFlags.CAMERA_SCC.value:
    ...
    if frame % 5 == 0:
      ...
      if CS.adrv_0x161 is not None:
        ...
        # ▼ [테스트 코드] 전방 차량이 인식되면 3번(선명한 흰색), 안 보이면 2번(회색)
        base_lane_color = 3 if hud_control.leadVisible else 2

        # --- 왼쪽 차선 ---
        if lane_line_check >= 1:
          lane_line_warn_left = CS.out.leftLaneLine % 10 not in (0, 5)
        else:
          lane_line_warn_left = CS.out.leftLaneLine >= 20
          
        # 사각지대나 실선 경고 시엔 4(노란색), 평소엔 위에서 정한 base_lane_color 적용
        lane_color_left = 4 if lane_line_warn_left or CS.out.leftBlindspot else base_lane_color
        
        if hud_control.leftLaneDepart:
          values["LANELINE_LEFT"] = 4 if (frame // 50) % 2 == 0 else 0  # 이탈 시 깜빡임
        else:
          values["LANELINE_LEFT"] = lane_color_left if hud_control.leftLaneVisible else 0
        ...
        lane_color_right = 4 if lane_line_warn_right or CS.out.rightBlindspot else base_lane_color
        
        if hud_control.rightLaneDepart:
          values["LANELINE_RIGHT"] = 4 if (frame // 50) % 2 == 0 else 0  # 이탈 시 깜빡임
        else:
          values["LANELINE_RIGHT"] = lane_color_right if hud_control.rightLaneVisible else 0
