  ...
  if CP.flags & HyundaiFlags.CAMERA_SCC.value:
    ...
    if frame % 5 == 0:
      ...
      if CS.adrv_0x161 is not None:
        ...
        values["DISTANCE_CAR"] = 2 if hdp_active else 3 if cruise_enabled and hud_control.leadVisible else 2 if cruise_enabled else 1 if main_enabled else 0      
        values["DISTANCE_SPACING"] = 1 if hdp_active else 1 if cruise_enabled and hud_control.leadVisible else 3 if cruise_enabled else 0
