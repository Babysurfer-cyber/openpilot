...
def create_lfahda_cluster(packer, CS, CAN, long_active, lat_active, hud_control):
  ...
  values["HDA_LFA_SymSta"] = 1 if lat_active else 0
...
def create_acc_control_scc2(packer, CAN, enabled, accel_last, accel, stopping, gas_override, set_speed, hud_control, hyundai_jerk, CS):
  ...
  # 앞차 인식 여부에 따른 타겟 거리 설정 (실제 거리 반영 + 80m 제한)
  if hud_control.leadVisible:
    # 콤마가 인식한 실제 거리(leadDistance)를 넣되, min 함수를 써서 80을 넘지 않게 합니다.
    values["TARGET_DISTANCE"] = min(hud_control.leadDistance, 80.0)
  else:
...
def _make_ccnc_values(values, CS, lat_active, frame, hud_control,
                     ...
                     blink_t=1.0):
  if lane_line:
    curvature = round(CS.out.steeringAngleDeg / 1.0)
